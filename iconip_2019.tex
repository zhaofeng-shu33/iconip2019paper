...
Next we compare our implementation of PSP with previous work \cite{RN3,RN4}. The comparison is done on Gaussian blobs dataset ($\abs{E}=O(\abs{V}^2)$) and a graph dataset with two hierarchical levels($\abs{E}=O(\abs{V}^{3/2})$). We use the CPU times to measure the practical speed of each implementation, the result in Fig.\ref{fig:er}.(a,b) shows our implementation is much faster than previous ones.
